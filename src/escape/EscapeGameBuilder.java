...
import escape.board.BoardWizard;
...
public class EscapeGameBuilder
{
    ...
    
    {
    	...
    	/*for (int i=0;i<gameInitializer.getLocationInitializers().length;i++) {
    		if((gameInitializer.getLocationInitializers()[i].locationType==LocationType.BLOCK|| gameInitializer.getLocationInitializers()[i].locationType==LocationType.EXIT ) && gameInitializer.getLocationInitializers()[i].player!=null) {
    			throw new EscapeException("Cannot Have an invalid location for a player");
    		}
    	}*/
    	
    	Board gameBoard = BoardWizard.createB(gameInitializer.getCoordinateType(),gameInitializer.getxMax()
    			,gameInitializer.getyMax(), gameInitializer.getLocationInitializers(),gameInitializer.getPieceTypes());//this.getGameInitializer().getPieceTypes());	
    	
    	
    	//EscapeGameManager egmi= new EscapeManagerImplement(gameInitializer.getCoordinateType(), gameInitializer.getxMax(),gameInitializer.getyMax(),gameInitializer.getLocationInitializers(),gameInitializer.getPieceTypes() , gameInitializer.getRules());
    	EscapeGameManager egmi= new EscapeManagerImplement(gameInitializer.getCoordinateType(),gameBoard,getGameInitializer().getRules());
    	System.out.println(gameInitializer.getCoordinateType());
